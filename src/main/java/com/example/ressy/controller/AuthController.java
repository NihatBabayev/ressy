...
public class AuthController {
    ...
    public ResponseEntity<ResponseModel<String>> signupUser(@RequestBody UserDTO userDTO,
        ...
        if (otpCode == null) {
            ...
            emailService.publishMessage("OTP code", "Your OTP code is " + randomOtp + " Don't share it with others. ", email); //with notification MS //made changes here \n are removed

            //            emailService.sendEmail(email, "OTP code", "Your OTP code is \n" + randomOtp + "\nDon't share it with others."); //old way
    ...
    public ResponseEntity<ResponseModel<String>> forgotPassword(@RequestParam(value = "email") String email,
        ...
        if (otpCode == null && otpCodeValid == null & newPassword == null) {
            String randomOtp = String.valueOf(new Random().nextInt(9000) + 1000);
            redisTemplate.opsForValue().set(email, randomOtp, 1, TimeUnit.MINUTES);
            ResponseModel<String> responseModel = new ResponseModel<>();

            emailService.publishMessage("OTP code", "Your OTP code is " + randomOtp + " Don't share it with others. ", email); //with notification MS
        ...
        } else if (otpCodeValid.equals("OTP_CODE_VALID") && newPassword != null) {
            userService.updatePassword(email, newPassword);
            String jwtToken = jwtService.generateToken(email);
            emailService.publishMessage("Successful Password Recovery", "We are delighted to inform you that your password has been successfully renewed. If you have any further questions or concerns, please feel free to reach out.", email); //with notification MS
